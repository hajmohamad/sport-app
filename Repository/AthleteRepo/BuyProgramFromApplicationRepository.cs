...
//             {
//                 var discountCode = await context.DiscountCodes.FirstOrDefaultAsync(x => x.Id == payment.DiscountCodeId.Value);
...
//         var publicDiscountAmount = coachService.CalculatePublicDiscountAmount();
//         var discountedBasePrice = Math.Max(0, originalPrice - publicDiscountAmount);
...
//                 await ValidateDiscountCode(coachService.Id, NormalizeDiscountCode(discountCodeValue));
//             if (!discountCodeValidation.Action)
//             {
//                 return discountCodeValidation;
//             }
//
//             var discountCode = discountCodeValidation.Result as DiscountCode;
//             if (discountCode is null)
//             {
//                 return new ApiResponse { Action = false, Message = "کد اشتباه است" };
//             }
//
//             discountCodeAmount = CoachMappers.CalculateDiscountAmount(discountedBasePrice, discountCode.DiscountType,
...
//                 FinalPrice = Math.Max(0, discountedBasePrice - discountCodeAmount),
//                 HasPublicDiscount = publicDiscountAmount > 0,
//                 AppliedDiscountCode = appliedDiscountCode,
//                 PublicDiscountType = coachService.PublicDiscountType?.ToString(),
//                 PublicDiscountPercent = coachService.PublicDiscountPercent
//             }
//         };
//     }
//
//     private async Task<ApiResponse> ValidateDiscountCode(int serviceId, string normalizedCode)
//     {
//         var discountCode = await context.DiscountCodes
//             .FirstOrDefaultAsync(x => x.CoachServiceId == serviceId && x.Code == normalizedCode && !x.IsDeleted);
//         if (discountCode is null)
//         {
//             return new ApiResponse { Action = false, Message = "کد اشتباه است" };
//         }
...
//     }
//
//     private async Task<int?> GetDiscountCodeId(int serviceId, string? code)
//     {
...
//             .Where(x => x.CoachServiceId == serviceId && x.Code == code && !x.IsDeleted)
//             .Select(x => (int?)x.Id)
//             .FirstOrDefaultAsync();
//     }
//
...
//         }
//
//         public async Task<ApiResponse> PreviewCheckout(string phoneNumber, int coachingServiceId, CheckoutDiscountRequestDto? checkoutDiscountRequestDto)
//         {
...
//
//             return await CalculateCheckoutPricing(coachService, checkoutDiscountRequestDto?.DiscountCode);
//         }
//
...
//             }
//
//             var pricing = pricingResult.Result as DiscountPreviewDto;
//             if (pricing is null || pricing.FinalPrice <= 0)
//             {
...
//                 DiscountCodeId = await GetDiscountCodeId(coachService.Id, pricing.AppliedDiscountCode),
